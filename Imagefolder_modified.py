...
class DatasetFolder(VisionDataset):
    ...
    def __getitem__(self, index):
        ...
        return (sample,sample2), target, index
...
class MulImagefolder_modified(DatasetFolder):
    """A generic data loader where the images are arranged in this way: ::
    ...
    def __init__(self, root, n_augmentations=8,transform=None, target_transform=None,
                 loader=default_loader, is_valid_file=None, cached=False, number=None):
    ...
    def __getitem__(self, index):
        ...
        imgs = [self.transform(sample) for _ in range(self.n_augmentations)]
        if self.target_transform is not None:
            target = self.target_transform(target)
